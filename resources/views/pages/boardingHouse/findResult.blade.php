...
@section('content')
    <div id="Background" class="absolute top-0 w-full h-[570px] rounded-b-[75px] bg-[linear-gradient(180deg,#F2F9E6_0%,#D2EDE4_100%)]"></div>
        <div id="TopNav" class="relative flex items-center justify-between px-5 mt-[60px]">
            <a href="{{ route('find.boardingHouse') }}" class="w-12 h-12 flex items-center justify-center shrink-0 rounded-full overflow-hidden bg-white">
        ...
        <section id="Result" class=" relative flex flex-col gap-4 px-5 mt-5 mb-9">
            @if($boardingHouses->isEmpty())
                <p class="text-center w-full">No boarding houses found.</p>
            @endif
            @foreach($boardingHouses as $boardingHouse)
                <!-- Card Kos -->
                <a href="{{ route('boardingHouse.slug', $boardingHouse->slug) }}" class="card">
                    <div class="flex rounded-[30px] border border-[#F1F2F6] p-4 gap-4 bg-white hover:border-[#91BF77] transition-all duration-300">
                        ...
                        <div class="flex flex-col gap-3 w-full">
                            ...
                            <hr class="border-[#F1F2F6]">
                            <p class="font-semibold text-lg text-ngekos-orange">Rp {{ number_format($boardingHouse->rooms->first()->price_per_month, 0, ',', '.') }}<span class="text-sm text-ngekos-grey font-normal">/bulan</span></p>
                        </div>
                    </div>
                </a>
            @endforeach
